...
const {UPDATE_PROFILE_API, DELETE_ACCOUNT_API, GET_PROFILE_API, UPDATE_PROFILE_PICTURE_API} = profileEndPoints
...
export function updateProfile(token, formData){
    return async(dispatch)=>{
        ...
        try{
            ...
            dispatch(setPhotographer({...response.data.photographer, image: userImage}))

            localStorage.setItem("photographer", JSON.stringify(response.data.photographer))
...
export function updateProfilePicture(token, formData){
    return async (dispatch) => {
        ...
        try{
            const response = await apiConnector('PATCH', UPDATE_PROFILE_PICTURE_API, formData, {Authorization: `Bearer ${token}`})
            ...
            localStorage.setItem("Photographer", JSON.stringify(response.data.updatedPhotographer))
        }
        catch(err){
