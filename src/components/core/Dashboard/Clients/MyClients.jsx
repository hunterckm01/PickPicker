...
const MyClients = () => {
  ...
  return (
    <section className="w-full min-h-[calc(100vh-131px)] bg-[linear-gradient(103deg,_#efdaffc0_2.13%,_#b458ff8f_104.3%)]">
      ...
      {/* Customer List */}
      <ul className="flex flex-col mt-[90px] gap-6 pb-10">
        {clients?.map((client) => (
          <li
            key={client._id}
            className="mx-[170px] border bg-[rgba(255,255,255,0.1)] border-[rgba(0,0,0,0.2)] rounded-[10px]  flex font-syne text-3xl items-center justify-between px-[30px] py-[18px]"
          >
            ...
            <div className="flex gap-2 items-center w-[120px] cursor-pointer" onClick = {()=>deleteCustomer(client._id)}>
              <TiUserDeleteOutline />
              <p>Delete</p>
            </div>
          </li>
      ...
      {customerModal && (
        <CustomerModal setCustomerModal={setCustomerModal} token={token} editClient = {editClient}/>
      )}
